<?php
    ...
    */
    if ( empty( $name ) || empty( $email ) || empty( $password ) || empty( $confirm_password )  || empty( $role) ) {
        setError( "All the fields are required.", '/manage_users_add' );
    } else if ( $password !== $confirm_password ) {
        //  make sure password is match
        setError( "The password is not match", '/manage_users_add' );
    } else if ( strlen( $password ) < 8 ) {
        // make sure the password length is at least 8 chars
        setError( "Your password must be at least 8 characters", '/manage_users_add' );
    } else {  
        ...
        if ( $user ) {
            setError("The email provided has already been used.","/manage_users_add");
        } else {
